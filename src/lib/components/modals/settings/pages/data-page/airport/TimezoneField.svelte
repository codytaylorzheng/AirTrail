  ...
  <Form.Control>
    {#snippet children({ props })}
      ...
      <div class="relative">
        ...
        {/if}
        <div class="absolute right-2 top-1/2 z-10 -translate-y-1/2 text-muted-foreground">
    ...
    >
      ...
      >
        {#each filtered as entry}
          <li
            use:melt={$option({ value: entry, label: entry })}
        ...
        {:else}
          <li class="relative cursor-pointer scroll-my-2 rounded-md p-2 bg-popover dark:bg-dark-1 border">
            {#if $inputValue}
              No timezones found.
            {:else}
